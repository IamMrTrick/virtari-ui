...
Virtari layout — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-layout
# or
pnpm add @virtari-packages/react-layout
# or
yarn add @virtari-packages/react-layout
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-layout";
```
