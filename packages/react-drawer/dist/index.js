...
function useDrawerDrag(config) {
  ...
  const recordSample = useCallback((size) => {
  ...
  const releasePointerCapture = useCallback((el, pointerId) => {
  ...
  const reset = useCallback((pointerId) => {
    const state = stateRef.current;
    if (!state) return;
  ...
  const moveSession = useCallback((main, cross, prevent) => {
    const state = stateRef.current;
...
function getDefaultHeaderVariant(direction) {
...
function Drawer({
  ...
  const [mountedIndicator, setMountedIndicator] = useState(null);
  ...
  const [internalSnap, setInternalSnap] = useState(resolvedDefaultSnap);
  ...
  return /* @__PURE__ */ jsx(DrawerProvider, { value: ctxValue, children: /* @__PURE__ */ jsx(DialogPrimitive.Root, { open, onOpenChange: handleOpenChange, modal: isModal, children }) });
}
...
var DrawerContent = forwardRef(function DrawerContent2({
  ...
  const pendingResizeSizeRef = useRef(null);
  ...
  const writeVisualSize = useCallback((sizePx, options) => {
    const contentEl = contentRef.current;
    const overlayEl = overlayRef.current;
    const wrapperEl = getWrapperEl();
...
var DrawerHandle = forwardRef(function DrawerHandle2({ className, placement, ...props }, forwardedRef) {
  const {
    direction,
    handleRef,
    activeSnapPoint,
...
var DrawerHeader = forwardRef(function DrawerHeader2({ className, variant, ...props }, forwardedRef) {
...
export { Drawer, DrawerBody, DrawerClose, DrawerContent, DrawerDescription, DrawerFooter, DrawerHandle, DrawerHeader, DrawerOverlay, DrawerTitle, DrawerTrigger };
