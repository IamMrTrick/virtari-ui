...
Virtari select — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-select
# or
pnpm add @virtari-packages/react-select
# or
yarn add @virtari-packages/react-select
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-select";
```
