...
Virtari bottom navigation — mobile-first tab bar with Material 3 / iOS / Floating / Underline variants, FAB, badges, safe-area insets, auto-hide, and animated indicator.
...
```bash
npm install @virtari-packages/react-bottom-nav
# or
pnpm add @virtari-packages/react-bottom-nav
# or
yarn add @virtari-packages/react-bottom-nav
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-bottom-nav";
```

### Import styles

```ts
import "@virtari-packages/react-bottom-nav/styles";
```
