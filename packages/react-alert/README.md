...
Virtari alert — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-alert
# or
pnpm add @virtari-packages/react-alert
# or
yarn add @virtari-packages/react-alert
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-alert";
```

### Import styles

```ts
import "@virtari-packages/react-alert/styles";
```
