...
Virtari text — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-text
# or
pnpm add @virtari-packages/react-text
# or
yarn add @virtari-packages/react-text
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-text";
```

### Import styles

```ts
import "@virtari-packages/react-text/styles";
```
