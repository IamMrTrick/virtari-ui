...
Virtari avatar — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-avatar
# or
pnpm add @virtari-packages/react-avatar
# or
yarn add @virtari-packages/react-avatar
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-avatar";
```

### Import styles

```ts
import "@virtari-packages/react-avatar/styles";
```
