...
Virtari dialog — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-dialog
# or
pnpm add @virtari-packages/react-dialog
# or
yarn add @virtari-packages/react-dialog
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-dialog";
```
