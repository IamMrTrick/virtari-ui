...
Virtari slider — accessible React component built on CSS variables and logical properties.
...
```bash
npm install @virtari-packages/react-slider
# or
pnpm add @virtari-packages/react-slider
# or
yarn add @virtari-packages/react-slider
```
...
```tsx
import { /* … */ } from "@virtari-packages/react-slider";
```

### Import styles

```ts
import "@virtari-packages/react-slider/styles";
```
