...
for (const [p,content] of [...outputs]) if (p.startsWith('ai/skills/')) {
  emit(`apps/docs/public/${p}`,content);
  emit(`packages/cli/skills/${p.slice('ai/skills/'.length)}`,content);
}
const stale = [];
...
for(const p of previous) if(!outputs.has(p)&&exists(p)) {
  ...
  else if(p.startsWith('ai/skills/')||p.startsWith('apps/docs/public/ai/skills/')||p.startsWith('packages/cli/skills/')) fs.unlinkSync(path.join(root,p));
