...
function main() {
  ...
  for (const bp of BREAKPOINTS) {
    const width = breakpointWidths.get(bp);
    if (!width) throw new Error(`Missing --vds-${bp} breakpoint`);
    out += `\n@layer utilities {\n`;
  ...
  const sourcePath = path.join(SRC, "index.css");
  const expandedPath = path.join(DIST, "_expanded.css");
  fs.writeFileSync(sourcePath, out);
  fs.writeFileSync(expandedPath, out);

  // Quick stats for humans.
  const base = rules.length;
  const total = base * (1 + BREAKPOINTS.length);
  console.log(
    `[utilities] generated ${base} base classes × ${1 + BREAKPOINTS.length} (base + ${BREAKPOINTS.length} breakpoints) = ${total} total → ${path.relative(PKG, sourcePath)}`
  );
}
...
## packages/core/src/utilities.css
